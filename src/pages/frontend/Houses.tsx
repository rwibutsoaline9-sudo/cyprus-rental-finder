...
const Houses = () => {
  useVisitorTracking();
  const [filters, setFilters] = useState<PropertyFiltersType>({ property_type: 'house' });
  ...
  const handleFiltersChange = (newFilters: PropertyFiltersType) => {
    setFilters({ ...newFilters, property_type: 'house' });
  };
  ...
  const handleHeaderSearch = (query: string) => {
    setFilters(prev => ({ ...prev, city: query, property_type: 'house' }));
  };
        ...
        <div className="container mx-auto px-2 sm:px-4 py-3 sm:py-4 md:py-6">
          <div className="mb-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h1 className="text-3xl font-bold">Family Houses in Cyprus</h1>
            </div>
            <p className="text-muted-foreground">
              Spacious family-friendly houses with gardens and great amenities
            </p>
          </div>
            ...
            <div className="flex-1">
              <div className="mb-4 sm:mb-6">
                <div className="flex items-center gap-2 text-muted-foreground text-sm sm:text-base">
                  ...
                  <span>
                    {loading ? 'Searching...' : `${properties.length} houses found`}
              ...
              {loading ? (
                <div className="flex items-center justify-center py-8 sm:py-12">
                  ...
                  <span className="ml-2 text-muted-foreground text-sm sm:text-base">Loading houses...</span>
                </div>
              ) : properties.length === 0 ? (
                <Card>
                  <CardContent className="text-center py-8 sm:py-12">
                    <Home className="h-8 w-8 sm:h-12 sm:w-12 mx-auto text-muted-foreground mb-3 sm:mb-4" />
                    <h3 className="text-base sm:text-lg font-semibold mb-2">No houses found</h3>
                    <p className="text-muted-foreground mb-4 text-sm sm:text-base">
                      Try adjusting your search filters to find more houses.
                    </p>
                  </CardContent>
                </Card>
              ) : (
